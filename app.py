    ...
    if os.path.exists("proyek_data.csv"):
        ...
        if st.session_state.role == "admin":
            with st.expander("📊 Export Laporan Bulanan ke Excel"):
                ...
                # Format Teknisi: Ganti koma dengan enter
        ...
        for i, row in f_df.sort_values(by="Tanggal", ascending=False).iterrows():
            with st.expander(f"📌 {row['Nama Proyek']} | 📅 {str(row['Tanggal'].date())}"):
                ...
                if st.session_state.role == "admin":
                    ...
                    if c2.button("🗑️ Hapus", key=f"del_{row['ID']}"): df.drop(i).to_csv("proyek_data.csv", index=False); st.rerun()
                    ...
                    if st.session_state.get(f"show_{row['ID']}", False):
                        with st.form(key=f"f_{row['ID']}"):
                            ...
                            if st.form_submit_button("Simpan Perubahan"):
                                ...
                                df.to_csv("proyek_data.csv", index=False); st.success("✅ Data diupdate!"); time.sleep(1); st.session_state[f"show_{row['ID']}"] = False; st.rerun()
